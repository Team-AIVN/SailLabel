...
> 아래 항목이 확정돼야 "어떤 버튼을 숨기고/막을지"(2e)와 백엔드 구멍 수정 범위가 결정됩니다.
> 각 항목의 **추천안**에 이견 없으면 그대로 채택, 있으면 다른 선택지를 고르거나 **결정** 칸에 기입해 주세요.

| # | 결정할 것 | 선택지 | 추천안 | 팀 결정 |
| --- | --- | --- | --- | --- |
| D1 | **프로젝트를 누가 생성?** | (a) WM·SA만 / (b) WM·SA·PM / (c) 누구나(현재) | **(a) WM·SA만** — 생성은 워크스페이스 관리 행위 | |
| D2 | **워크스페이스를 누가 생성?** | (a) SA만 / (b) SA·WM / (c) 누구나(현재) | **(a) SA만** — 최상위 관리 단위, SA가 만들고 WM 배정 | |
| D3 | **PM이 프로젝트를 삭제 가능?** | (a) 불가(WM·SA만) / (b) 가능(현재) | **(a) 불가** — Notion ⚠️ 의도에 맞춤 | |
| D4 | **PM이 데이터셋 업로드 가능?** (명세 모순) | (a) 불가(WM만, 명세의 PM 설명 수정) / (b) 가능 | **(a) 불가** — 데이터셋은 워크스페이스 자산 | |
| D5 | **지급 기록을 누가 생성?** | (a) WM + 자기 프로젝트 PM / (b) WM만 / (c) 워크스페이스 멤버 누구나(현재) | **(a) WM + 자기 프로젝트 PM** | |
| D6 | **보상 단가 정책을 누가 설정?** | (a) WM·SA만 / (b) PM도 가능(현재) | **(a) WM·SA만** — 예산 정책은 상위 권한 | |
| D7 | **PMb(작업 미할당 프로젝트 멤버)를 지원?** | (a) 미지원(현재) / (b) 지원(모델에 무작업 `member` 역할 추가) | **(a) 미지원** — 당장 필요성 낮음, 필요 시 추후 | |
| D8 | **생성자에게 역할 자동 배정?** | (a) 프로젝트 생성자→PM, 워크스페이스 생성자→WM / (b) 배정 안 함(현재) | **(a) 자동 배정** — 생성자가 자기 것에 권한 없는 문제 해소 | |
| D9 | **SA를 무엇으로 정의?** (임시: org 소유자) | (a) org 소유자(현재) / (b) 명시적 `super_admin` 역할 / (c) Django superuser | **(a) org 소유자 유지** | |

### 확정 후 반영 위치
- **D1, D2**: `projects.create` / `workspaces.create` predicate 오버라이드 (`*/rules.py`) — 현재 구멍 메움
- **D3**: `projects.delete` predicate를 WM·SA로 좁힘
- **D4, D6**: 해당 API `permission_required` 조정 + `permissions.md`의 PM 설명 수정
- **D5**: `PaymentRecord` POST 권한을 `workspaces.view` → 관리 역할로 변경
- **D7**: (지원 시) `ProjectMember.role`에 무작업 역할 추가 + 마이그레이션
- **D8**: 프로젝트/워크스페이스 생성 시 `perform_create`에서 생성자 멤버십 자동 생성
- **D9**: `users/rules.py` `is_super_admin` (현재 org 소유자 조건 유지/변경)

> 확정되면 이 결정들을 `permissions.md`·`permissions-by-url.md`에 반영하고, 프론트 게이팅(2e)은 이 기준표대로 버튼 노출/비활성을 구현합니다.
